...
struct VertexBufferGL {
	void Create( const std::shared_ptr<Memory> mem, const VertexLayoutHandle layoutHandle );
	void Destroy();
...
struct IndexBufferGL {
	void Create( const std::shared_ptr<Memory> mem );
	void Destroy();
...
struct ShaderGL {
	void Create( const std::shared_ptr<Memory> mem, GLenum shaderType );
	void Destroy();
