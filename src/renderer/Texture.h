...
public:
    ...
    std::shared_ptr<ITexture2D> LoadTexture2DFromMemory(const char *name, uint32_t width, uint32_t height, EPixelFormat format, const void *pixels);

private:
    std::shared_ptr<ITexture2D> FindImage(uint32_t hashKey);
    std::shared_ptr<ITexture2D> ImageFromMemoryInternal(uint32_t hashKey, uint32_t width, uint32_t height, EPixelFormat format, const void *pixels);
